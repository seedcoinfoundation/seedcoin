    ...
    cachedNumBlocks(0)
{
    ...
    fSeedcoinCheckBalanceChanged = false;
...
void WalletModel::pollBalanceChanged()
{
    ...
    if(fSeedcoinCheckBalanceChanged || nBestHeight != cachedNumBlocks || nDarksendRounds != cachedDarksendRounds || cachedTxLocks != nCompleteTXLocks)
    {
        fSeedcoinCheckBalanceChanged = false;
...
{
    // Balance and number of transactions might have changed
    fSeedcoinCheckBalanceChanged = true;
}
...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CSeedcoincoinAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CSeedcoincoinAddress(rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CSeedcoincoinAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CSeedcoincoinAddress(strAddress).Get();
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(CSeedcoincoinAddress(address).ToString());
...
{
    // Seedcoin context; old object no longer relocks wallet
    *this = rhs;
    rhs.relock = false;
}
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CSeedcoincoinAddress(address).ToString())].push_back(out);
    }
}
