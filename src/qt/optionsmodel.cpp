...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSeedcoinAmount"))
        settings.setValue("nAnonymizeSeedcoinAmount", 1000);
    nAnonymizeSeedcoinAmount = settings.value("nAnonymizeSeedcoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSeedcoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeSeedcoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeSeedcoinAmount:
            return QVariant(nAnonymizeSeedcoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeSeedcoinAmount:
            nAnonymizeSeedcoinAmount = value.toInt();
            settings.setValue("nAnonymizeSeedcoinAmount", nAnonymizeSeedcoinAmount);
            emit AnonymizeSeedcoinAmountChanged(nAnonymizeSeedcoinAmount);
            break;
        default:
            break;
        }
