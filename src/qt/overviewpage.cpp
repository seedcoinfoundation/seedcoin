...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSeedcoinAmount = BitcoinUnits::formatWithUnit(false, nAnonymizeSeedcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeSeedcoinAmount = strAnonymizeSeedcoinAmount.remove(strAnonymizeSeedcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSeedcoinAmount + " XSD / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSeedcoinAmount*COIN) nMaxToAnonymize = nAnonymizeSeedcoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSeedcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeSeedcoinAmount));
        strAnonymizeSeedcoinAmount = strAnonymizeSeedcoinAmount.remove(strAnonymizeSeedcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSeedcoinAmount + " XSD / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
